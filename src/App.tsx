...
export default function App(){
 ...
 const remove=()=>{if(layers.length<=1||!active)return;snapshot();const ls=layers.filter(l=>l.id!==active.id);setLayers(ls);setSelected(ls[0].id)};
  ...
  <div className="workspace">
   <aside className="leftbar">
    ...
    <div className="leftpanel">
      ...
      panel==="assets"?<div className="assetspanel"><div className="paneltitle">ASSETS</div><button className="assetcard" onClick={()=>file.current?.click()}><Upload size={18}/><div><b>Import design</b><span>PNG · JPG · WEBP</span></div></button><div className="miniassets"><button onClick={addPattern}><div className="patternicon stripes"/><span>Stripes</span></button><button onClick={()=>{setPattern("checker");addPattern()}}><div className="patternicon checker"/></button><button onClick={()=>{setPattern("gradient");addPattern()}}><div className="patternicon gradient"/></button></div><input hidden ref={file} type="file" accept="image/*" onChange={e=>e.target.files?.[0]&&importImage(e.target.files[0])}/></div>:
